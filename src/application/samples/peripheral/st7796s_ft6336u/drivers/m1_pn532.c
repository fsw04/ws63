...
#define MIFARE_DEFAULT_KEY_LEN        M1_KEY_LEN
...
void m1_pn532_get_i2c_levels(uint8_t *scl, uint8_t *sda)
{
    soft_i2c_scl_release();
    soft_i2c_sda_release();
    soft_i2c_delay();
    if (scl != NULL) {
        *scl = (uint8_t)uapi_gpio_get_val(CONFIG_M1_PN532_SOFT_I2C_SCL_PIN);
    }
    if (sda != NULL) {
        *sda = (uint8_t)uapi_gpio_get_val(CONFIG_M1_PN532_SOFT_I2C_SDA_PIN);
    }
}
...
static m1_error_t m1_pn532_auth_with_key(const m1_card_t *card, uint8_t block, uint8_t key_type,
                                         const uint8_t key[M1_KEY_LEN])
{
    ...
    if (card == NULL || key == NULL || card->uid_len == 0 || card->uid_len > M1_UID_MAX_LEN) {
        return M1_ERR_PARAM;
    }
    ...
    payload[len++] = block;
    memcpy(&payload[len], key, MIFARE_DEFAULT_KEY_LEN);
...
m1_error_t m1_pn532_auth_key_a_with_key(const m1_card_t *card, uint8_t block, const uint8_t key[M1_KEY_LEN])
{
    return m1_pn532_auth_with_key(card, block, MIFARE_AUTH_KEY_A, key);
}

m1_error_t m1_pn532_auth_key_a(const m1_card_t *card, uint8_t block)
{
    return m1_pn532_auth_with_key(card, block, MIFARE_AUTH_KEY_A, g_mifare_default_key);
}

m1_error_t m1_pn532_auth_key_b(const m1_card_t *card, uint8_t block)
{
    return m1_pn532_auth_with_key(card, block, MIFARE_AUTH_KEY_B, g_mifare_default_key);
}
